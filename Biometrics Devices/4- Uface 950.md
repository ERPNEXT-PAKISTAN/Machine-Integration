...
# ERPNext Config
# ----------------------------------------
ERPNEXT_URL = "192.168.xxx.xxx"
ERPNEXT_API_KEY = "4545x54x5xec0"
ERPNEXT_API_SECRET = "xx5454545x55x9f5"
...
logging.info("ADMS Listener Loaded")

# ----------------------------------------
# Helper: Send to ERPNext
# ----------------------------------------
def send_to_erpnext(employee_id, timestamp, log_type, device_id):
    payload = {
        "employee": employee_id,          # biometric_id in ERPNext Employee
        "time": timestamp,                # datetime
        "log_type": log_type,             # IN / OUT
        "device_id": device_id
    }
    ...
    try:
        ...
        logging.error(f"[ERPNext Exception] {e}")
        return 500


# ----------------------------------------
# Device → /iclock/cdata (Main Attendance Endpoint)
# ----------------------------------------
...
def iclock_cdata():
    ...
    if table == "ATTLOG":
        ...
        for row in rows:
            ...
            pin = parts[0].strip()          # employee UID
            timestamp = parts[1].strip()    # timestamp string

            # Convert timestamp to ERPNext format
            try:
                ts = datetime.strptime(timestamp, "%Y-%m-%d").strftime("%Y-%m-%d %H:%M:%S")
            except:
                ts = timestamp  # already full datetime

            # Log type is always IN (ZKTeco does not send OUT)
            log_type = "IN"

            send_to_erpnext(pin, timestamp, log_type, params.get("SN", ""))
...
def iclock_getrequest():
    logging.info(f"[GETREQUEST] {dict(request.args)}")
    return "OK"
