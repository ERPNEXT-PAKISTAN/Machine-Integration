...
API_KEY = "515d55cba61eec0"
API_SECRET = "826dc77489739f5"
DEVICE_SN = "BRMC224260030"

logging.basicConfig(filename="/opt/zk_adms/adms.log",
                    level=logging.INFO,
...
def cdata():
    ...
    logging.info(f"[cdata] params={params} body={body}")

    # ATTLOG FORMAT (typical)
    # ex: "2025-11-22 16:37:30\t101\t1\t0"

    if "table" in params and params["table"] == "ATTLOG":
        ...
        for line in lines:
            ...
            if len(parts) >= 2:
                ...
                payload = {
                    "employee": uid,  # ERPNext will map via biometric_id
                    "time": punch_time,
                    "log_type": "IN",
                    "device_id": DEVICE_SN
                }
                ...
                try:
                    ...
                    logging.info(f"[ERPNext] {r.status_code}: {r.text}")

                except Exception as e:
                    logging.error(f"[ERPNext Error] {e}")

    return "OK"

@app.route("/iclock/getrequest", methods=['GET'])
def getrequest():
    logging.info(f"[getrequest] params={request.args.to_dict()}")
    return "OK"
