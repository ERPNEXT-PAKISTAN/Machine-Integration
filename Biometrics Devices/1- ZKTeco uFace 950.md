...
# === CONFIG ===
ERPNEXT_URL = "192.168.12.25:8000"
API_KEY = "51565656661eec0"
API_SECRET = "82656565669739f5"
LISTEN_PORT = int(os.environ.get("ADMS_PORT", "5000"))

# Optional: if you want to restrict to known device serial numbers:
ALLOWED_DEVICE_SN = ["BRMC224260030"]  # add more SN strings as needed

LOGFILE = "/opt/zk_adms/adms.log"
logging.basicConfig(filename=LOGFILE, level=logging.INFO,
                    format="%(asctime)s %(levelname)s %(message)s")

def erp_request(method, path, json=None, params=None):
    headers = {
        "Authorization": f"token {API_KEY}:{API_SECRET}",
        "Content-Type": "application/json"
    }
    url = ERPNEXT_URL.rstrip("/") + path
    resp = requests.request(method, url, headers=headers, json=json, params=params, timeout=15, verify=True)
    return resp

@app.route("/device/verify", methods=["GET", "POST"])
def device_verify():
    try:
        # ADMS devices often send GET with query params like: uid, sn, name, time
        data = request.values.to_dict()
        logging.info(f"Received ADMS data: {data}")

        uid = data.get("uid") or data.get("id") or data.get("user_id")
        time_val = data.get("time") or data.get("date") or data.get("timestamp")
        sn = data.get("sn") or data.get("serial") or data.get("device_sn")

        if not uid:
            logging.warning("No uid found in request, ignoring.")
            return "OK", 200

        if ALLOWED_DEVICE_SN and sn and sn not in ALLOWED_DEVICE_SN:
            logging.warning(f"Received from unknown device SN: {sn}")
            return "OK", 200

        # Normalize time: if time missing, use current UTC
        if time_val:
            # ADMS often sends "YYYY-MM-DD HH:MM:SS" or epoch. Try to pass as-is to ERPNext.
            checkin_time = time_val
        else:
            checkin_time = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")

        # 1) find employee in ERPNext by custom field 'biometric_id' (you will create this field)
        # Query Employee doctype
        filter_expr = [['Employee','biometric_id','=', uid]]
        params = {
            "filters": str(filter_expr).replace("'", '"'),
            "fields": '["name","employee_name"]'
        }
        resp = erp_request("GET", "/api/resource/Employee", params=params)
        if resp.status_code != 200:
            logging.error(f"Failed to query Employee: {resp.status_code} {resp.text}")
            # still return OK to the device
            return "OK", 200

        emp_data = resp.json().get("data") or []
        if not emp_data:
            logging.warning(f"No employee found for biometric uid={uid}")
            # Optionally: create a placeholder Employee or log and return OK
            return "OK", 200

        employee_docname = emp_data[0]["name"]

        # 2) Create Employee Checkin
        payload = {
        ...
        r = erp_request("POST", "/api/resource/Employee Checkin", json=payload)
        if r.status_code in (200, 201):
            logging.info(f"Created checkin for {employee_docname} at {checkin_time}")
        else:
            logging.error(f"Failed to create checkin: {r.status_code} {r.text}")

        return "OK", 200

    except Exception as e:
        logging.exception("Exception in /device/verify")
        return "ERROR", 500
